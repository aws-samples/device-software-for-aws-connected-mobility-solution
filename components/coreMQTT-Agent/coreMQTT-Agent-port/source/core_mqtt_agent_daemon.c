...
/**
 * @file core_mqtt_agent_daemon.c
 * @brief This file contains functions to initialize, connect MQTT and start MQTT agent daemon.
 */
 ...
 */
#if 0	/* This is left as an optimization reference. */
 ...
 * used in this example.
 */
static void prvConnectAndStartDaemonTask( void * pvParameters );
...
/* Multiple transport network is supported in this platform.
 * The network context must be allocated from TLS_FreeRTOS_AllocContet.
 */
static NetworkContext_t * pGlobalNetworkContext = NULL;
 ...
 */
void startMqttAgentTask( void )
{
    /* prvConnectAndStartDaemonTask() connects to the MQTT broker, then turns
     * itself into the MQTT agent task. */
    xTaskCreate( (TaskFunction_t) prvConnectAndStartDaemonTask,
                 "ConnectManager",
...
static MQTTStatus_t prvMQTTInit( void )
{
    ...
    xTransport.pNetworkContext = pGlobalNetworkContext;
...
static BaseType_t prvSocketConnect( NetworkContext_t * pxNetworkContext )
{
    ...
    const TickType_t xTransportTimeout = 0UL;
        ...
        /* Set the credentials for establishing a TLS connection. */
            ...
            xNetworkCredentials.rootCaSize = ( uint32_t )( root_cert_auth_pem_end - root_cert_auth_pem_start );
        #endif
            ...
            xNetworkCredentials.clientCertSize = ( uint32_t )( client_cert_pem_end - client_cert_pem_start );
        #endif
            ...
            xNetworkCredentials.privateKeySize = ( uint32_t )( client_key_pem_end - client_key_pem_start );
        #endif
                                        ...
                                        MQTTPublishInfo_t * pxPublishInfo )
{
    ...
    if( xPublishHandled != true )
    {
        ...
        *pcLocation = 0x00;
        LogWarn(( "WARN:  Received an unsolicited publish from topic %.*s", pxPublishInfo->topicNameLength, pxPublishInfo->pTopicName ));
...
static void prvMQTTAgentTask( void * pvParameters )
{
    ...
    NetworkContext_t *pNetworkContext = pGlobalNetworkContext;
    ...
    do
    {
        ...
        if( xMQTTStatus == MQTTSuccess )
        {
            ...
            xNetworkResult = prvSocketDisconnect( pNetworkContext );
        }
        /* Error. */
        else
        {
            /* Reconnect TCP. */
            LogError(( "MQTT Agent process loop failed with error: %d.", xMQTTStatus ));
            LogWarn(( "Reconnecting socket and mqtt." ));
            xNetworkResult = prvSocketDisconnect( pNetworkContext );
            configASSERT( xNetworkResult == pdPASS );
            xNetworkResult = prvSocketConnect( pNetworkContext );
            configASSERT( xNetworkResult == pdPASS );
...
static void prvConnectToMQTTBroker( void )
{
    ...
    /* Allocate the network context. */
    pGlobalNetworkContext = TLS_FreeRTOS_AllocContext();
    assert( pGlobalNetworkContext != NULL );
    pNetworkContext = pGlobalNetworkContext;

    /* Connect a TCP socket to the broker. */
    xNetworkStatus = prvSocketConnect( pNetworkContext );
